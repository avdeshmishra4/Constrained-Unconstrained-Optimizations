...
%	and the problem m-file: f.m

%  the problem file for plotting f_plot.m
...
function ReturnValue = SimulatedAnnealing(functname,dvar0,niter,tol,lowbound,intvl,ntrials)
...
%
	...
	set(gcf,'Name','Contour for f(x1, x2)')
	...
	title('Contour Plot and Moves for Simulated Annealing')
...
%if abs(beta0) > 2.5 % change to be made here
...
for i = 1:niter-1
   ...
   slope = (feval(functname,nextx)-feval(functname,x))/hstep;
   ...
   if(slope < 0) 
      ...
      if (nvar == 2)
        ...
        line([xs(is,1) xs(is+1,1)],[xs(is,2) xs(is+1,2)], 'LineWidth',2,'Color','r')
      ...
      if (rpnew <= pnew)   % let it escape
         ...
         as(is+1) = 0;
      	...
      	if (nvar == 2)
        		...
        		line([xs(is,1) xs(is+1,1)],[xs(is,2) xs(is+1,2)], 'LineWidth',2,'Color','b')
